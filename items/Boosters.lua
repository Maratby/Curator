...
SMODS.Booster {
    ...
    config = { extra = 4, choose = 1, curated = { mod_name = "RandomMod", mod_id = "randommod"} },
    ...
    set_ability = function(self, card, initial, delay_sprites)
        if Curator.joker_table then
            ...
            if temp then
                ...
                self.config.curated.mod_id = temp.NormalisedName
            end
        end
    end,
    create_card = function(self, card, i)
        local tempset = "cura_curated_pack_pool_" .. self.config.curated.mod_id
